...
def shuffleAndSplitDataByClass_distillation(dataX, dataY, cluster):
    ...
    print(
        "total number of samples: {}, total number of classes:{}, number of samples per class:{}".format(len(dataYList),
                                                                                                         len(n_class),
                                                                                                         cluster_for_one_class))
    ...
    for label in n_class:
        ...
        print(
            "class {} has {}, {}, {}, {} samples for targetModel and shadowModel, {} samples for distillation.".format(
                label, len(toTrainDataIndex), len(shadowDataIndex), len(toTestDataIndex), len(shadowTestDataIndex),
                len(distillationDataIndex)))
    ...
    # 保存索引到文件
    data_index_path = "./data/data_index.pkl"
    with open(data_index_path, "wb") as f:
        pickle.dump(
            {
                "toTrainDataIndexTotal": toTrainDataIndexTotal,
                "shadowDataIndexTotal": shadowDataIndexTotal,
                "toTestDataIndexTotal": toTestDataIndexTotal,
                "shadowTestDataIndexTotal": shadowTestDataIndexTotal,
                "distillationDataIndexTotal": distillationDataIndexTotal,
            },
            f,
        )
...
def initializeTargetModelforDistill(dataset, num_epoch, dataFolderPath='./data/',
                                    modelFolderPath='./model_IncludingDistillation/', classifierType='cnn',
                                    num_epoch_for_distillation=50):
    ...
    attackModelDataTarget, attackModelLabelsTarget, targetModelToStore, classification_y = att_frame.trainTarget(
        classifierType, targetTrain, targetTrainLabel, X_test=targetTest, y_test=targetTestLabel, splitData=False,
        inepochs=num_epoch, batch_size=100, datasetFlag=dataset)
...
def initializeShadowModelforDistill(dataset, num_epoch, dataFolderPath='./data/',
                                    modelFolderPath='./model_IncludingDistillation/', classifierType='cnn',
                                    num_epoch_for_distillation=50):
    dataPath = dataFolderPath + dataset + '/PreprocessedIncludingDistillationData'
    attackerModelDataPath = dataFolderPath + dataset + '/attackerModelDataIncludingDistillation'
    ...
    attackModelDataShadow, attackModelLabelsShadow, shadowModelToStore, classification_y = att_frame.trainTarget(
        classifierType, shadowTrain, shadowTrainLabel, X_test=shadowTest, y_test=shadowTestLabel, splitData=False,
        inepochs=num_epoch, batch_size=100, datasetFlag=dataset)
...
def initializeDataIncludingDistillationData(dataset, orginialDatasetPath, dataFolderPath='./data/'):
    if (dataset == 'CIFAR10'):
        ...
        toTrainData, toTrainLabel, shadowData, shadowLabel, toTestData, toTestLabel, shadowTestData, shadowTestLabel, distillationData, distillationDataLabel = shuffleAndSplitDataByClass_distillation(
            X, y, cluster)
        ...
        print("Loading official data from the local disk (CIFAR100)")
        dataX, dataY, testdataX, testdataY = rd.readCIFAR100(orginialDatasetPath)
        X = np.concatenate((dataX, testdataX), axis=0)
        y = np.concatenate((dataY, testdataY), axis=0)
        print("Preprocessing data, there is {} samples in the dataset".format(len(X)))
        cluster = 10000
        ...
        print("Loading official data from the local disk (CINIC10)")
        dataX, dataY, testdataX, testdataY = rd.readCINIC10(orginialDatasetPath)
        X = np.concatenate((dataX, testdataX), axis=0)
        y = np.concatenate((dataY, testdataY), axis=0)
        print("Preprocessing data, there is {} samples in the dataset".format(len(X)))
        cluster = 10000
        ...
        print("Loading official data from the local disk (SVHN)")
        dataX, dataY, testdataX, testdataY = rd.readSVHN(orginialDatasetPath)
        X = np.concatenate((dataX, testdataX), axis=0)
        y = np.concatenate((dataY, testdataY), axis=0)
        print("Preprocessing data, there is {} samples in the dataset".format(len(X)))
        cluster = 16548
...
def createAttackDataWithMetrics(dataset, dataFolderPath='./data/', modelFolderPath='./model_IncludingDistillation/',
                                classifierType='cnn', TargetOrShadow='Target', batch_size=100, metricFlag='loss'):
    ...
    if TargetOrShadow == 'Target':
        np.savez(attackerModelDataPath + '/targetModelData_{}.npz'.format(classifierType), attack_x, attack_y,
                 classification_y, losses)
    elif TargetOrShadow == 'Shadow':
        np.savez(attackerModelDataPath + '/shadowModelData_{}.npz'.format(classifierType), attack_x, attack_y,
                 classification_y, losses)

    return attack_x, attack_y, classification_y, losses


def distill_original_model(modelPath, original_model, modelType, distill_dataset, epochs=100, batch_size=100,
                           learning_rate=0.01, l2_ratio=1e-7,
    ...
    if (datasetFlag == 'CIFAR10' or datasetFlag == 'CIFAR100' or datasetFlag == 'CINIC10' or datasetFlag == 'SVHN'):
        ...
        distill_loader_with_softlabels_noshuffle = DataLoader(distill_data_with_softlabels, batch_size=batch_size,
                                                              shuffle=False)
    ...
    if modelType == 'vgg':
        ...
        net = net.to(device)
    elif modelType == 'mobilenetv2':  # 如果选择 MobileNetV2
    ...
    elif modelType == 'resnet50':  # 如果选择 MobileNetV2
        print('Using ResNet50 model...')
        # 配置 ResNet50 模型的参数
        params['init_weights'] = True
        net = models.ResNet50(params)  # 使用 ResNet50 类
        net = net.to(device)
    elif modelType == 'densenet121':  # 使用 DenseNet121
    ...
    if modelType in ['mobilenetv2', 'vgg', 'resnet50', 'densenet121']:
        l2_ratio = 0.0005
        learning_rate = 0.1
    ...
    print(
        'dataset: {},  model: {},  device: {},  epoch: {},  batch_size: {},   learning_rate: {},  l2_ratio: {}'.format(
            datasetFlag, modelType, device, epochs, batch_size, learning_rate, l2_ratio))
...
def DistillModel(original_model, dataset, num_epoch, dataFolderPath='./data/',
                 modelFolderPath='./model_IncludingDistillation/', classifierType='cnn', TargetOrShadow='Target'):
    dataPath = dataFolderPath + dataset + '/PreprocessedIncludingDistillationData'
    ...
    distill_original_model(modelPath, original_model, classifierType, distill_dataset, epochs=num_epoch, batch_size=100,
                           learning_rate=0.001, l2_ratio=1e-7,
                           n_hidden=128, datasetFlag=dataset, TargetOrShadow=TargetOrShadow)

    return modelPath


def generateAttackDataForSeqMIA(dataset, classifierType, dataFolderPath, pathToLoadData, num_epoch, preprocessData,
                                trainTargetModel, trainShadowModel, topX=3, num_epoch_for_distillation=50,
                                distillTargetModel=True, distillShadowModel=True, metricFlag='loss'):
    if (preprocessData):
        initializeDataIncludingDistillationData(dataset, pathToLoadData)
    if (trainTargetModel):
        targetModel = initializeTargetModelforDistill(dataset, num_epoch, classifierType=classifierType,
                                                      num_epoch_for_distillation=num_epoch_for_distillation)
    else:
    ...
    if (distillTargetModel):
        distillTargetModelPath = DistillModel(targetModel, dataset, num_epoch_for_distillation,
                                              classifierType=classifierType,
                                              modelFolderPath='./model_IncludingDistillation/', TargetOrShadow='Target')
    else:
        modelFolderPath = './model_IncludingDistillation/'
        distillTargetModelPath = modelFolderPath + dataset + '/' + 'Target'
    if (trainShadowModel):
        shadowModel = initializeShadowModelforDistill(dataset, num_epoch, classifierType=classifierType,
                                                      num_epoch_for_distillation=num_epoch_for_distillation)
    else:
        modelFolderPath = './model_IncludingDistillation/'
        modelPath = modelFolderPath + dataset
    ...
    if (distillShadowModel):
        distillShadowModelPath = DistillModel(shadowModel, dataset, num_epoch_for_distillation,
                                              classifierType=classifierType,
                                              modelFolderPath='./model_IncludingDistillation/', TargetOrShadow='Shadow')
    else:
        modelFolderPath = './model_IncludingDistillation/'
        distillTargetModelPath = modelFolderPath + dataset + '/' + 'Shadow'
    targetX, targetY, target_classification_y, target_losses = createAttackDataWithMetrics(dataset,
                                                                                           dataFolderPath='./data/',
                                                                                           modelFolderPath='./model_IncludingDistillation/',
                                                                                           classifierType=classifierType,
                                                                                           TargetOrShadow='Target',
                                                                                           batch_size=100,
                                                                                           metricFlag=metricFlag)

    shadowX, shadowY, shadow_classification_y, shadow_losses = createAttackDataWithMetrics(dataset,
                                                                                           dataFolderPath='./data/',
                                                                                           modelFolderPath='./model_IncludingDistillation/',
                                                                                           classifierType=classifierType,
                                                                                           TargetOrShadow='Shadow',
                                                                                           batch_size=100,
                                                                                           metricFlag=metricFlag)
